...
def compute_hausdorff(test, reference):
    """
    description: test和reference之间的HD & HD95
    param:
    test (numpy.ndarray): 待推理数据集, shape = (1, 155, 240, 240)。
    reference (numpy.ndarray): 参考数据集, shape = (1, 155, 240, 240)。
    ...
    for i in range(test_slices.shape[0]):  # 遍历每个切片(155)
        slice_test = test_slices[i]
        slice_reference = reference_slices[i]

        # 计算Hausdorff距离
        if slice_test.any() and slice_reference.any():  # 确保非空
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        hd, hd95 = compute_hausdorff(test=mask_pred, reference=mask_ref)
        if tp + fp + fn == 0:
...
if __name__ == '__main__':
    folder_ref = r'/root/autodl-tmp/nnUNet/nnUNet_raw/Dataset725_BraTS/labelsTr'  #labelsTr路径
    folder_pred = r'/root/autodl-tmp/nnUNet/nnUNet_results/Dataset725_BraTS/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_4/validation'
    output_file = r'/root/autodl-tmp/nnUNet/nnUNet_results/Dataset725_BraTS/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_4/validation/summary.json'
